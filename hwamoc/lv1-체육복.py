def solution(n, lost, reserve):
    ...
    for i in lost:      # 체육복 없는 학생은 -1 해준다 (하나도 없는 학생은 0이 된다)
        students[i-1] -= 1

    for i in reserve:       # 여벌의 체육복을 가져온 학생은 +1 해준다
        students[i-1] += 1  #(여벌의 체육복이 있고 도둑맞지 않은 학생은 2가 된다)

    for i, v in enumerate(students):    # index와 학생이 갖고 있는 체육복 갯수를 tuple형태로 반환
        if v == 0:      # 체육복을 도난당한 학생일 때
            if i > 0 and students[i-1] == 2:    # 앞 번호의 학생이 여벌을 가지고 있으면
                students[i-1] -= 1              # 앞 번호 학생에게 빌려서 (앞 번호 학생 값에 -1)
                students[i] += 1                # 입는다 (도난 당한 학생 값에 +1)
