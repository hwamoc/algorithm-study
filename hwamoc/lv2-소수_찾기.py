...
def solution(numbers):
    ...
    for i in range(2, n + 1):
        if a[i]:    # a[i] 소수일 때 true 이므로 아래 실행 (초기에 2 들어감. 다음은 3 들어감)
            primes.append(i)    # 소수를 primes 배열에 추가
            for j in range(2 * i, n + 1, i):    # 소수의 배수들을 걸러준다. 소수의 2배수 부터 시작 n+1까지, i씩 커진다
...
def solution(n):
    ...
    for i in range(len(n)):                                                 # for문을 돌면서
        a |= set(map(int, map("".join, permutations(list(n), i + 1))))      # i+1개의 원소로 순열을 만든 결과를 set a와 합집한(OR연산)한 뒤 결과를 a에 할당
    a -= set(range(0, 2))   # 0, 1을 세트로 만들어 빼준 결과를 다시 a에 할당

    for i in range(2, int(max(a) ** 0.5) + 1):      # 최대값의 제곱근 까지만 for문을 돈다
        a -= set(range(i * 2, max(a) + 1, i))       # 에라토스테네스의 체: 소수의 배수들을 걸러준다. 소수의 2배수 부터 시작 n+1까지, i씩 커진다
    return len(a)
'''
